...
export default function CompetencyReportPage() {
    ...
    const [allowedEmpIds, setAllowedEmpIds] = useState<Set<string> | null>(null)
        ...
        async function init() {
            ...
            if (profile?.employee_id) {
                const { data: empInfo } = await supabase
                    .from('employees').select('role, department').eq('employee_id', profile.employee_id).single()
                if (empInfo) {
            ...
            // Build scoped employee list for non-admins
            ...
            if (userRole !== 'admin' && userDept) {
                setAllowedEmpIds(new Set((emps || []).map(e => e.employee_id)))
            }
        }
        init()
    }, [])
    ...
    const generateReport = useCallback(async () => {
        ...
        // Use plain select('*') — avoids FK join dependency that fails when
        // the Supabase schema cache hasn't picked up the relationship yet.
        let query = supabase
        ...
        if (reportType === 'expiring') {
            ...
            // Logic: either valid_till is in the future/today OR it's null (permanent)
            query = query.or(`valid_till.gte.${todayStr},valid_till.is.null`)
        }
        // 'history' — no status/date filter; returns all records
        ...
        // Sort: fewest days first for expiring/expired; newest first for active/history
        ...
        // Resolve names from the already-loaded allEmployees list (no extra fetch needed)
        ...
        const processed: ProcessedRow[] = (data || [])
            ...
            .filter((r: ProcessedRow) => !allowedEmpIds || allowedEmpIds.has(r.employee_id))

        setRows(processed)
        setLoading(false)
    }, [reportType, department, designation, threshold, empFilter, allowedEmpIds, allEmployees])
