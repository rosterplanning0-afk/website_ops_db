...
interface InstructionAckSheetClientProps {
    initialInstructions: InstructionOption[]
    userContext: { role: string; dept: string; employeeId: string }
}

export function InstructionAckSheetClient({ initialInstructions, userContext }: InstructionAckSheetClientProps) {
    const [instructions, setInstructions] = useState<InstructionOption[]>(initialInstructions)
    const [selectedId, setSelectedId] = useState(initialInstructions[0]?.id || '')
